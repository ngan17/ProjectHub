            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="alert alert-info border-0 mb-4">
                        ...
                        Danh sách các nhóm trong lớp <strong>{{ $userClasses->class_name }}</strong>
                    </div>

                    <div class="row g-3">
                        @forelse($userClasses->groups as $classGroup)
                            @php
                                $isMember = $classGroup->members->contains('user_id', Auth::id()) || $classGroup->leader_id == Auth::id();
                                $hasPendingRequest = $classGroup->joinRequests()->where('member_id', Auth::id())->where('status', 'Pending')->exists();
                            @endphp
                            ...
                            <div class="col-md-6">
                                <div class="card h-100 {{ $isMember ? 'border-success' : '' }}">
                                    <div class="card-body">
                                        ...
                                        @else
                                            <form action="{{ route('user.join-request.send') }}" method="POST">
                                                @csrf
